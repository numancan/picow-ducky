...
static inline void kb_set_poll_ms(uint32_t ms) {
    if (ms >= INTERVAL_MS_MIN) interval_ms = ms;
}

bool kb_is_buffer_empty() { return kb_buffer.counter == 0; }
...
void kb_send_reports() {
    ...
    if (!is_key_pressed) {
        if (kb_has_keycode()) {
            ...
            kb_report_chr(*kb_buffer.pString++);
...
uint8_t kb_str_to_keycode(uint8_t* str) {
    ...
    for (uint8_t i = 0; i < len; i++) {
        if (strcmp(str_conv_table[i].str, str) == 0) return str_conv_table[i].keycode;
    }
